# IMPORTS → HELPERS → PRODUCTS-CREATE → PRODUCTS-READ → PRODUCTS-UPDATE → PRODUCTS-DELETE → PRODUCTS-ROUTER → ORDERS-PLACE

# ============================================================
#  IMPORTS
# ============================================================
import json   # for reading the request
from django.views.decorators.csrf import csrf_exempt  # bec django blocks post, put, delete from other origins by default for security
from django.shortcuts import render
from django.http import JsonResponse      # django tool for sending back json
from .db import products_collection, orders_collection  # . -> from the same folder as this file, getting collections from shop
...
# ============================================================
#  HELPERS  (serialize, soft-delete filter, validation)
# ============================================================
...
def exclude_deleted(query_filter):         # modifies a filter dict before it gets sent to db
...
def validate_product_data(data, require_all=True):
    ...
    # 6. Check stock (whole number, 0 or more, OPTIONAL)
    if "stock" in data:
...
def create_product(request):
    try:
        data = json.loads(request.body)          # the new product comes in the request body as JSON, not URL. we turn raw json into python dict
    ...
    new_product = {
        ...
        "description": data.get("description", ""),
        "image": data.get("image", ""),
        "stock": data.get("stock", 0),  # use the stock given or default to 0
        "is_deleted": False,
    }
...
# ============================================================
#  PRODUCTS - READ  (list, retrieve one, retrieve by query)
# ============================================================
def list_products(request):
    ...
    if search:
        query["name"] = {"$regex": search, "$options": "i"}  # flexible text search similar to contains in sql, i = ignore upper/lowercase
...
# ============================================================
#  PRODUCTS - UPDATE  (PUT = full replace, PATCH = partial)
# ============================================================
def update_product(request, product_id):
    ...
    if request.method == "PUT":
        ...
        allowed_fields = ["name", "price", "category", "description", "image", "stock"]
        fields_to_update = {}
...
# ============================================================
#  PRODUCTS - ROUTER  (dispatches /products/<id>/ by method)
# ============================================================
@csrf_exempt
...
# ============================================================
#  ORDERS - PLACE  (stock check -> reduce stock -> save order)
# ============================================================
@csrf_exempt
